...
export function findPath<T extends any[],>(tree: T, func: Fn, config: Partial<TreeHelperConfig> = {},) {
  config = getConfig(config,)
  const path = []
  const list = [...tree,]
 ...
 */
export function treeMap<T = any,>(treeData: T[], opt: { children?: string, conversion: Fn },): { [p: string]: any }[] {
